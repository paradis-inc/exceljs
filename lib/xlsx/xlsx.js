...
class XLSX {
  ...
  async load(data, options) {
    ...
    for (const entry of Object.values(zip.files)) {
      ...
      if (!entry.dir) {
        ...
        // externalLinks は raw buffer として保持
        if (entryName.match(/xl\/externalLinks\//)) {
          const buf = await entry.async('nodebuffer');
          model.externalLinkEntries[entryName] = buf;
          continue;
        }
        ...
        switch (entryName) {
          ...
          case 'xl/workbook.xml': {
            ...
            // externalReferences を保持する
            if (workbook.externalReferencesXml) {
              model.externalReferencesXml = workbook.externalReferencesXml;
            }
            break;
          }
          ...
          default: {
            ...
            if (match) {
              ...
              model.rawVmlDrawings[`../drawings/${match[1]}.vml`] = await entry.async('string');
            ...
            if (match) {
              ...
              model.rawVmlDrawingRels[`../drawings/${match[1]}.vml`] = await entry.async('string');
              break;
            }
          }
        }
  ...
  async addMedia(zip, model) {
    const cameraEMFNames = model.cameraEMFNames || new Set();
    await Promise.all(
      model.media.map(async medium => {
        if (medium.type === 'image') {
          // Skip camera EMF snapshots so Excel re-renders the camera on open
          if (medium.extension === 'emf' && cameraEMFNames.has(medium.name)) {
            return;
          }
  ...
  addDrawings(zip, model) {
    ...
    // Collect EMF media names used exclusively by camera shapes.
    // Excluding them from the zip forces Excel to re-render the camera
    // content from the live cell range when the file is opened.
    model.cameraEMFNames = new Set();
    ...
    model.worksheets.forEach(worksheet => {
      ...
      if (drawing) {
        if (worksheet.rawDrawingXml) {
          ...
          zip.append(worksheet.rawDrawingXml, {name: `xl/drawings/${drawing.name}.xml`});
          if (worksheet.rawDrawingRelsXml) {
            zip.append(worksheet.rawDrawingRelsXml, {name: `xl/drawings/_rels/${drawing.name}.xml.rels`});
            // Find camera shape rIds and their EMF media names
            XLSX.collectCameraEMFNames(
              worksheet.rawDrawingXml,
              worksheet.rawDrawingRelsXml,
              model.cameraEMFNames,
            );
          } else {
  ...
  /**
   * Parse a drawing XML and its rels XML to find EMF media names referenced
   * exclusively by camera shapes (<xdr:pic> with <a14:cameraTool>).
   * These names are added to the provided Set so addMedia can skip them,
   * forcing Excel to re-render the camera content from the live cell range.
   */
  static collectCameraEMFNames(drawingXml, relsXml, result) {
    const picRegex = /<xdr:pic>[\s\S]*?<\/xdr:pic>/g;
    let picMatch;
    while ((picMatch = picRegex.exec(drawingXml)) !== null) {
      const picXml = picMatch[0];
      if (!picXml.includes('cameraTool')) continue;
      const rIdMatch = picXml.match(/r:embed="(rId\d+)"/);
      if (!rIdMatch) continue;
      const rId = rIdMatch[1];
      // Find Target in rels XML for this rId
      const relMatch = relsXml.match(
        new RegExp(`Id="${rId}"[^>]*Target="[^"]*/([^"/]+\\.emf)"`)
      );
      if (relMatch) {
        const filename = relMatch[1]; // e.g. "image1.emf"
        result.add(filename.replace(/\.[^.]+$/, '')); // e.g. "image1"
      }
    }
  }
  ...
  async addWorkbookRels(zip, model) {
    ...
    // externalLinks の Relationship を追加
    if (model.externalLinkEntries && Object.keys(model.externalLinkEntries).length > 0) {
      // externalLink ファイルを Target 名でソートして順序を安定させる
      const extLinkFiles = Object.keys(model.externalLinkEntries)
        .filter(p => p.match(/xl\/externalLinks\/externalLink\d+\.xml$/))
        .sort();
      model._externalLinkRidMap = {};
      extLinkFiles.forEach(filePath => {
        const newRid = `rId${count++}`;
        const target = filePath.replace('xl/', '');
        model._externalLinkRidMap[filePath] = newRid;
        relationships.push({
          Id: newRid,
  ...
  async addExternalLinks(zip, model) {
    if (!model.externalLinkEntries) return;
    // externalLink ファイル本体を zip に追加
    for (const [filePath, buf] of Object.entries(model.externalLinkEntries)) {
      zip.append(buf, {name: filePath});
    }
    // externalLinks の _rels ファイルも保持されている場合は追加
    // (既に externalLinkEntries に含まれているはず)
  }

  async addWorkbook(zip, model) {
    const xform = new WorkbookXform();
    // externalReferences の raw XML を新しい rId にマッピングして書き換える
    if (model.externalReferencesXml && model._externalLinkRidMap) {
      const ridMap = model._externalLinkRidMap;
      // テンプレートの rId -> 新 rId のマッピングを作成
      // テンプレートの externalLink ファイルパスを順番通りに取得
      const extLinkFiles = Object.keys(ridMap).sort();
      // externalReferences XML から既存の rId を抽出
      const existingRids = [];
      const rIdPattern = /r:id="([^"]+)"/g;
      let m;
      while ((m = rIdPattern.exec(model.externalReferencesXml)) !== null) {
        existingRids.push(m[1]);
      }
      // 既存 rId を新 rId に置換（属性値境界を正規表現で確実に一致させる）
      let newXml = model.externalReferencesXml;
      existingRids.forEach((oldRid, i) => {
        const filePath = extLinkFiles[i];
        if (filePath && ridMap[filePath]) {
          newXml = newXml.replace(
            new RegExp(`r:id="${oldRid.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"`, 'g'),
            `r:id="${ridMap[filePath]}"`
          );
        }
      });
      model.externalReferencesXml = newXml;
    }
  ...
  async addWorksheets(zip, model) {
    ...
    model.worksheets.forEach(worksheet => {
      ...
      if (worksheet.rawVmlXml) {
        ...
        zip.append(worksheet.rawVmlXml, {name: `xl/drawings/vmlDrawing${worksheet.id}.vml`});
        if (worksheet.rawVmlRelsXml) {
          zip.append(worksheet.rawVmlRelsXml, {name: `xl/drawings/_rels/vmlDrawing${worksheet.id}.vml.rels`});
        }
