...
class XLSX {
  ...
  addDrawings(zip, model) {
    ...
    model.worksheets.forEach(worksheet => {
      ...
      if (drawing) {
        if (worksheet.effectiveDrawingXml) {
          // Use raw XML to preserve camera shapes and other unrecognized drawing elements.
          // effectiveDrawingXml applies picture visibility (removes hidden xdr:pic anchors).
          zip.append(worksheet.effectiveDrawingXml, {name: `xl/drawings/${drawing.name}.xml`});
  ...
  async refreshCameras(xlsxBuffer, handler) {
    ...
    for (const drawingPath of drawingFiles) {
      ...
      const relsPath = `xl/drawings/_rels/${drawingBaseName}.xml.rels`;
      ...
      const findAnchors = (xml, tagName) => {
        const results = [];
        const re = new RegExp(`<xdr:${tagName}[\\s\\S]*?<\\/xdr:${tagName}>`, 'g');
        let m;
      ...
      const anchors = [
        ...findAnchors(drawingXml, 'twoCellAnchor'),
        ...findAnchors(drawingXml, 'oneCellAnchor'),
      ];
      ...
      for (const block of anchors) {
        ...
        const newMediaName = `${baseName}.png`;
        const newTarget = oldTarget.replace(mediaName, newMediaName);

        const oldMediaPath = `xl/media/${mediaName}`;
        if (zip.files[oldMediaPath]) {
          zip.remove(oldMediaPath);
        }
        zip.file(`xl/media/${newMediaName}`, pngBuffer);

        // rels の Target を更新（同じ rId のエントリのみ）
        relsXml = relsXml.replace(
          new RegExp(`(<Relationship[^>]*\\bId="${rId}"[^>]*\\bTarget=)"[^"]*"`),
          `$1"${newTarget}"`,
        );
        relsModified = true;
        anyModified = true;
