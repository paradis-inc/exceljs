...
class CNvPrXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    // extLstXml が保存されている場合は ExtLstXform の render を通してそのまま出力
    const extLstModel = model.extLstXml ? {extLstXml: model.extLstXml} : model;
    this.map['a:extLst'].render(xmlStream, extLstModel);
    xmlStream.closeNode();
  }
  ...
  parseClose(name) {
    ...
    switch (name) {
      case this.tag: {
        ...
        const extLstModel = this.map['a:extLst'].model;
        if (extLstModel && extLstModel.extLstXml) {
          this.model = this.model || {};
          this.model.extLstXml = extLstModel.extLstXml;
        }
        return false;
      }
      default:
