...
const {needsQuoting} = require('../.././../doc/range');
  ...
  prepare(model) {
    ...
    model.sheets.forEach(sheet => {
      if (sheet.pageSetup && sheet.pageSetup.printArea) {
        const definedName = {
          ...
          ranges: sheet.pageSetup.printArea.split('&&').map(printArea => {
            ...
            const sheetRef = needsQuoting(sheet.name) ? `'${sheet.name}'` : sheet.name;
      ...
      if (sheet.pageSetup && (sheet.pageSetup.printTitlesRow || sheet.pageSetup.printTitlesColumn)) {
        const ranges = [];
        const sheetRefTitles = needsQuoting(sheet.name) ? `'${sheet.name}'` : sheet.name;
