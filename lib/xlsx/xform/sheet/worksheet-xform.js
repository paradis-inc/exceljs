...
const imageAnchorSignature = medium =>
  JSON.stringify({
    imageId: medium.imageId,
    range: medium.range && {
      tl: medium.range.tl,
      br: medium.range.br,
      ext: medium.range.ext,
      editAs: medium.range.editAs,
    },
    hyperlinks: medium.hyperlinks,
    spPr: medium.spPr,
  });
...
class WorkSheetXform extends BaseXform {
  ...
  prepare(model, options) {
    ...
    // background media は先に処理（drawing.anchors には含まれない）
    ...
    // anchors 配列が存在する場合は元の順序を維持しつつ、
    // 読み込み後に addImage() された画像を末尾にマージする
    const baseAnchors = model.anchors && model.anchors.length > 0 ? model.anchors.slice() : [];
    const existingImageAnchors = new Set(
      baseAnchors
        .filter(entry => entry.kind === 'image' && entry.item)
        .map(entry => imageAnchorSignature(entry.item))
    );
    const appendedImageAnchors = model.media
      .filter(m => m.type === 'image')
      .filter(m => !existingImageAnchors.has(imageAnchorSignature(m)))
      .map(m => ({kind: 'image', item: m}));

    const anchorSource = baseAnchors.length > 0
      ? [...baseAnchors, ...appendedImageAnchors]
      : [
          ...model.media.filter(m => m.type === 'image').map(m => ({kind: 'image', item: m})),
          ...model.shapes.map(s => ({kind: 'shape', item: s})),
        ];

    anchorSource.forEach(entry => {
      if (entry.kind === 'image') {
        const medium = entry.item;
        bookImage = options.media[medium.imageId];
        ...
        // spPr (xfrm) を保持する
        if (medium.spPr) {
          anchor.picture.spPr = medium.spPr;
        }
        ...
        if (shape.name !== undefined) {
          anchorShape.name = shape.name;
        }
        if (shape.visible !== undefined) {
          anchorShape.visible = shape.visible;
        }
        if (shape.shapeType) {
          anchorShape.shapeType = shape.shapeType;
        }
        // txBox と spLocks を保持する
        if (shape.txBox !== undefined) {
          anchorShape.txBox = shape.txBox;
        }
        if (shape.spLocks !== undefined) {
          anchorShape.spLocks = shape.spLocks;
        }
        if (shape.extLstXml !== undefined) {
          anchorShape.extLstXml = shape.extLstXml;
        }
        if (shape.style !== undefined) {
          anchorShape.style = shape.style;
        }
        const anchor = {
          shape: anchorShape,
          range: shape.range,
        ...
        const groupItem = entry.item;
        const drawing = getDrawingModel();
        drawing.anchors.push({
          range: groupItem.range,
          rawXmls: groupItem.rawXmls,
        });
      }
    });
  ...
  render(xmlStream, model) {
    ...
    if (model.rels) {
      // add a <legacyDrawing /> node for each comment vmlDrawing
  ...
  reconcile(model, options) {
    ...
    const rels = (model.relationships || []).reduce((h, rel) => {
      ...
      if (rel.Type === RelType.Comments) {
        const commentData = options.comments && options.comments[rel.Target];
        if (commentData) {
          model.comments = commentData.comments;
        }
      }
      if (rel.Type === RelType.VmlDrawing) {
        ...
        if (model.comments && model.comments.length) {
          ...
          if (vmlComment) {
            model.comments.forEach((comment, index) => {
              comment.note = Object.assign({}, comment.note, vmlComment.comments[index]);
            });
          }
        }
      }
      ...
      if (match) {
        ...
        drawing.anchors.forEach(anchor => {
          if (anchor.medium) {
            ...
            // picture の spPr (xfrm) を保持する
            if (anchor.picture && anchor.picture.spPr) {
              image.spPr = anchor.picture.spPr;
            }
            model.media.push(image);
            model.anchors.push({kind: 'image', item: image});
            ...
            if (anchor.shape.name !== undefined) {
              shape.name = anchor.shape.name;
            }
            if (anchor.shape.visible !== undefined) {
              shape.visible = anchor.shape.visible;
            }
            if (anchor.shape.index !== undefined) {
              shape.index = anchor.shape.index;
            }
            if (anchor.shape.shapeType) {
              shape.shapeType = anchor.shape.shapeType;
            }
            // txBox と spLocks を保持する
            if (anchor.shape.txBox !== undefined) {
              shape.txBox = anchor.shape.txBox;
            }
            if (anchor.shape.spLocks !== undefined) {
              shape.spLocks = anchor.shape.spLocks;
            }
            if (anchor.shape.extLstXml !== undefined) {
              shape.extLstXml = anchor.shape.extLstXml;
            }
            if (anchor.shape.style !== undefined) {
              shape.style = anchor.shape.style;
            }
            model.shapes.push(shape);
            model.anchors.push({kind: 'shape', item: shape});
          } else if (anchor.rawXmls) {
            // グループ（xdr:grpSp）等の raw XML anchor
            model.anchors.push({kind: 'group', item: {range: anchor.range, rawXmls: anchor.rawXmls}});
          }
        });
      }
    }
