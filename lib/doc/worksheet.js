...
class Worksheet {
  ...
  hidePictureByName(name) {
    if (!this.rawDrawingXml) return false;
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const namePattern = new RegExp(`<xdr:cNvPr\\b[^>]*\\bname="${escaped}"`);
    let found = false;
    const removeMatchingAnchor = (match) => {
    ...
    this.rawDrawingXml = this.rawDrawingXml.replace(/<xdr:twoCellAnchor[\s\S]*?<\/xdr:twoCellAnchor>/g, removeMatchingAnchor);
    this.rawDrawingXml = this.rawDrawingXml.replace(/<xdr:oneCellAnchor[\s\S]*?<\/xdr:oneCellAnchor>/g, removeMatchingAnchor);
    return found;
  }
...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`
  ...
  _resolveShape(shapeRef) {
    ...
    if (typeof shapeRef === 'string') {
      return this.shapes.find(shape => getShapeName(shape) === shapeRef);
    }
    return undefined;
  }
